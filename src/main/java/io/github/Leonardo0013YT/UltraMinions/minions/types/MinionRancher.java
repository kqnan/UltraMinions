...
public class MinionRancher extends Minion {

    public MinionRancher(Main plugin, YamlConfiguration minion, String path, File f) {
    ...
    public void update(PlayerMinion pm, ArmorStand armor, PlayerMinionStat stat, Location spawn, CallBackAPI<Boolean> action) {
        ...
        Collection<Entity> ents = spawn.getWorld().getNearbyEntities(spawn, Main.get().getCfm().getX() * 1.5, Main.get().getCfm().getY() * 1.5, Main.get().getCfm().getZ() * 1.5);
